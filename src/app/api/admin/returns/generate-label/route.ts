  ...
  try {
    ...
    const cartPayload = {
      ...
      to: {
        ...
        phone: process.env.COMPANY_PHONE?.replace(/\D/g, "") || "13991257795",
        email: process.env.COMPANY_EMAIL || "contato@oficialbricks.com.br",
        document:
          process.env.COMPANY_DOCUMENT?.replace(/\D/g, "") || "31604350830",
        address: process.env.COMPANY_ADDRESS || "Rua Oswaldo Cochrane",
        complement: process.env.COMPANY_COMPLEMENT || "",
        number: process.env.COMPANY_NUMBER || "294",
        district: process.env.COMPANY_DISTRICT || "Embaré",
        city: process.env.COMPANY_CITY || "Santos",
        state_abbr: process.env.COMPANY_STATE || "SP",
        country_id: "BR",
        postal_code: process.env.COMPANY_CEP?.replace(/\D/g, "") || "11040-111",
      },
