...
export async function POST(request: NextRequest) {
  try {
    ...
    if (order.status !== 'return_approved') {
      return NextResponse.json(
        { error: 'Este pedido precisa estar aprovado para gerar etiqueta de devolução' },
        { status: 400 }
      );
    }
    ...
    const quoteResponse = await fetch(`${MELHOR_ENVIO_URL}/me/shipment/calculate`, {
      method: 'POST',
      headers: {
    ...
    const selectedQuote = quotes.find((q: any) => !q.error && q.name === 'PAC') || 
                          quotes.find((q: any) => !q.error);
    ...
    const checkoutResponse = await fetch(`${MELHOR_ENVIO_URL}/me/shipment/checkout`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    const generateResponse = await fetch(`${MELHOR_ENVIO_URL}/me/shipment/generate`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    return NextResponse.json(
      { error: 'Erro ao processar geração de etiqueta', details: error instanceof Error ? error.message : 'Erro desconhecido' },
      { status: 500 }
    );
  }
}
